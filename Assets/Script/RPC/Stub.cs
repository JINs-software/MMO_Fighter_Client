...
public abstract class Stub_FightGameDamage : Stub
{
    ...
    {
        int offset = 0;
        UInt32 attker = BitConverter.ToUInt32(payload, offset); offset += sizeof(UInt32);
        UInt32 target = BitConverter.ToUInt32(payload, offset); offset += sizeof(UInt32);
        byte targetHP = payload[offset++];
        DAMAGE(attker, target, targetHP);
    }

    protected abstract void DAMAGE(UInt32 attker, UInt32 target, byte targetHP);
...
public abstract class Stub_FightGameComm : Stub
{
    ...
    {
        int offset = 0;
        UInt32 Time = BitConverter.ToUInt32(payload, offset); offset += sizeof(UInt32);
        ECHO(Time);
    }
